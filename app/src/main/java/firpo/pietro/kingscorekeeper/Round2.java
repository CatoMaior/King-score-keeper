...
public class Round2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Round.setText("Seconda mano");
        TextView WhatToTake = (TextView) findViewById(R.id.WhatToTake);
        WhatToTake.setText("No Regine\nMeno 3 punti ogni presa");
        ...
        for (int i = 0; i < 4; i++){
            ...
            PlayerBars[i].setMax(4);
        }

        ForwardButton = (Button) findViewById(R.id.ForwardButton);
        BackButton = (Button) findViewById(R.id.BackButton);

        /*int[] totalPoints = PlayerNames.getTotalPoints();
        int[] ranking = totalPoints;
        Arrays.sort(ranking);
        for(int i = 0; i < ranking.length / 2; i++) {
            int temp = ranking[i];
            ranking[i] = ranking[ranking.length - i - 1];
            ranking[ranking.length - i - 1] = temp;
        }
        for (int positionInRanking = 0; positionInRanking < 4; positionInRanking++){
            for (int positionInPoints = 0; positionInPoints < 4; positionInPoints++){
                if(ranking[positionInRanking] == totalPoints[positionInPoints] && !isPlayerinRanking[positionInPoints]){
                    RankingPlayerNames[positionInRanking].setText(PlayerNames.getPlayers()[positionInPoints]);
                    RankingPlayerPoints[positionInRanking].setText(String.valueOf(ranking[positionInRanking]));
                    isPlayerinRanking[positionInPoints] = true;
                    break;
                }
            }
        }*/
        ...
        PlayerBars[0].setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                if(isSumFour()) ForwardButton.setEnabled(true);
        ...
        ForwardButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int pointsForToken = Settings.getRound2Points();
                PlayerNames.setRoundPoints(2, PlayerBars[0].getProgress() * pointsForToken,
                        PlayerBars[1].getProgress() * pointsForToken,
                        PlayerBars[2].getProgress() * pointsForToken,
                        PlayerBars[3].getProgress() * pointsForToken);

                Intent OpenNextRound = new Intent(Round2.this, Round3.class);
    ...
    private static boolean isSumFour(){
        int sum = 0;
        for (int i = 0; i < 4; i++){
